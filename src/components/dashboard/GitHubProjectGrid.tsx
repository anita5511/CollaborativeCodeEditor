...
import { Github, Star, MoreVertical, Clock, GitBranch, Folder } from 'lucide-react';
...
interface GitHubProject {
  ...
  name: string;
  description?: string;
  status: 'imported' | 'cloning' | 'ready' | 'error';
  created_at: string;
  updated_at: string;
  file_count?: number;
  github_url?: string;
...
export const GitHubProjectGrid: React.FC<GitHubProjectGridProps> = ({ projects, loading }) => {
  if (loading) {
    return (
  ...
  if (projects.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <h3 className="text-lg font-medium text-white mb-2">No GitHub projects yet</h3>
        <p className="text-gray-400 mb-6">Import your first GitHub repository to get started</p>
      </div>
    );
  }
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
      {projects.map((project) => {
        ...
        const ProjectCard = ({ children }: { children: React.ReactNode }) => {
          if (isClickable) {
            return (
              <Link to={`/github-project/${project.id}`} className="block">
                {children}
              </Link>
            );
          }
            ...
            <div className={`bg-gray-800 rounded-lg border border-gray-700 transition-all duration-200 group ${
              ...
              <div className="p-4">
                ...
                <div className="w-full h-32 bg-gray-900 rounded-lg mb-4 flex items-center justify-center border border-gray-700 relative overflow-hidden">
                  <Github className={`w-8 h-8 text-gray-500 ${isClickable ? 'group-hover:text-blue-400' : ''} transition-colors`} />
